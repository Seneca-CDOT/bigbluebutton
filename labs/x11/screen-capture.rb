...
class Headless
  class VideoRecorder
    ...
    def start_capture
      ...
      cl = "ffmpeg -f openal -i 'Monitor of Dummy Output'  -f flv rtmp://dev110.bigbluebutton.org/screenshare/foo/room2"
      cl = "ffmpeg -y -r 5 -s 1280x1024 -f x11grab -i :#{@display} -vcodec libx264 -preset ultrafast -pix_fmt yuv420p -b 300k -f flv rtmp://dev110.bigbluebutton.org/screenshare/foo/room2"
      cl = "ffmpeg -f openal -i 'Monitor of Dummy Output' -framerate 12 -s 1920x1080 -f x11grab -i :#{@display}+12,77 -g 24 -preset veryfast -tune zerolatency -r 12  -c:v libx264 -crf 24 -pix_fmt yuv420p -b 300k -f flv rtmp://dev110.bigbluebutton.org/screenshare/foo/room2"

      puts cl

      CliUtil.fork_process(cl, @pid_file_path, @log_file_path)
      # CliUtil.fork_process(command_line_for_capture, @pid_file_path, @log_file_path)
      at_exit do
